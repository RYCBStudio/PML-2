...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            // Only launch a new process if one isn't already running.
            // This prevents the terminal from restarting when switching tabs
            // in a TabControl, which unloads/reloads the control content.
            if (!string.IsNullOrEmpty(Process) && _ptyConnection == null)
            {
                LaunchProcess();
            }
        ...
        {
            _cursorBlinkTimer.Stop();
            // NOTE: We intentionally do NOT clean up the PTY process or unsubscribe
            // terminal events here. In a TabControl, switching tabs unloads/reloads
            // the control content, which would otherwise kill the running process.
            // The process will be cleaned up when the tab is explicitly closed
            // (by sending "exit" to the PTY) or when the application exits.
        }
        ...
        public async Task SendToPtyAsync(string data, CancellationToken ct = default)
        {
