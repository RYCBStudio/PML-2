...
「证书助手」基于 lego 手动 DNS-01 流程，常见问题如下。

### lego 下载失败

- **表现**：点「开始申请」后长时间停留在「正在准备 lego…」，或直接提示无法准备 lego。
- **原因**：网络无法访问 GitHub（主源）与备用镜像，或下载内容校验不通过。
- **处理**：确认网络与代理后重试；应用会依次尝试主源与备用源。校验失败通常意味着下载不完整，重试即可。

### 一直停留在「请添加 TXT 记录后继续」

- **这是正常等待状态**：应用在等您到 DNS 服务商添加 TXT 记录，不会自动超时。
...
- **证书已过期**：隧道表单会在距到期不足 30 天时提醒（Staging 证书也会提示）。
